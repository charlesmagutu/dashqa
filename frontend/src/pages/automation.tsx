...
const Automation : React.FC<ApplicationDetailsProps> = () => {
  ...
  const getRuns = useCallback(async () => {
    try {
      ...
      const availableRuns = await response.json();
      setRuns(availableRuns);
  
      // Automatically set the latest run ID
      if (availableRuns.length > 0) {
        const latestRunId = availableRuns[0].id; // Assuming `availableRuns` is sorted by date
  ...
  const fetchData = useCallback(async () => {
    try {
      ...
      const fetchId = id || selectedItem?.runId; // Prioritize URL param over state
      if (!fetchId) {
        setData([]);
        return;
      }
  
      const response = await fetch(`http://localhost:8081/api/v1/test/${fetchId}`);
      if (!response.ok) {
        throw new Error(`HTTP error! status: ${response.status}`);
      }
  ...
  const formatDateTime = (datetime: string | number | Date) => new Date(datetime).toLocaleString();
    ...
    <Card className="w-full">
      <CardHeader>
        <div className="flex justify-between items-center">
          <CardTitle>  {selectedItem?.runId ? `Test Results for Run Id: ${selectedItem.runId}` : 'Test Results'}</CardTitle>
          ...
          >
            ...
            {runs.map((run) => (
              <option key={run.id} value={run.id}>
                {run.id} ({new Date(run.runDate).toLocaleString()})
              </option>
            ))}
          </select>
        </div>
